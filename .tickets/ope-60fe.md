...
**2026-07-29T23:12:52Z**

Steps 4, 7, 8, 9 LOCAL WORK COMPLETE. Everything below is committed locally and UNPUSHED; every remaining action is one that leaves the machine, i.e. the owner's.

STEP 4 -- shared library repo built at ~/Desktop/openhound-collector-common: 51 tracked files (61 previously tracked, plus .gitignore and ci.yml, minus the 12 the BloodHound removal took out), commit 58667ea, remote set to Mayyhem/openhound-collector-common. Runbook's '63 files' corrected to 51. It now PASSES ITS OWN CI STANDALONE -- ruff clean, mypy clean on 29 files, 81 tests, wheel builds -- which is what step 2d existed to prove and what exposed the dependency bug below.

BUG FOUND BY STEP 4 (fixed, fork commit aff9126): the library imported openhound, pydantic and duckdb at module level in src/ and declared NONE of them. openhound + pydantic in graph/stub_node.py and graph/graph_edge.py (they subclass the framework's BaseAsset and dataclass entry models -- so graph/ is NOT framework-agnostic, and now says so); duckdb in dlt/duckdb_safe.py. It never showed because the library had only ever been installed into a collector's venv, which supplied all three transitively. Published as-is, anyone installing openhound-collector-common without a collector would have hit ModuleNotFoundError. Also capped ruff <0.16 in BOTH packages: 0.16 widened ruff's DEFAULT rule selection past E4/E7/E9/F, taking the library from clean to 173 findings with no code change (103 UP045), so an unpinned floor makes the first fresh uv lock turn CI red.

STEP 7 -- SpecterOps/ConfigManBearPig cloned; root matched the runbook's 8 items exactly, zero tags, no open PRs. Annotated tag v1.2-powershell created locally on the pristine tree (script self-reports $script:ScriptVersion = '1.2'). Independently verified both git rm claims before deleting: the unit-test script is byte-identical (both blob 20c27f9613d3) and the collector's 23 cypher queries are a strict superset of upstream's 17.

STEP 8/9 -- new tree assembled and committed as a260c36. PowerShell files git-mv'd to powershell_deprecated/, superseded unit-test script + cypher_queries removed, collector tree overlaid, .tickets/ + fork-root plans/specs copied. 446 additions, 9 renames, 10 modifications. Audited clean: no out/, .sdd/, .venv, *.duckdb, bloodhound-upload/, and the scrubbed ope-1f49 carries no credential. Step 9's four fixes applied: uv.sources ../../ -> ../, three README claims (Quick Start now leads with 'uv tool install', the git-URL framework claim replaced with 'openhound>=0.2.12 from PyPI', tree diagram rerooted), CLAUDE.md + AGENTS.md authored identically (shared-library paragraph rewritten for it being a published package, plus a Project Conventions section covering tickets/logging/tests/docs/schema files), and grill-me given .agents/skills/grill-me/SKILL.md of its own.

TWO MORE FINDINGS in step 8/9:
(1) BLOCKER, fixed (fork commit 1a1f12e): the v1.2-powershell tag makes the whole repo UNBUILDABLE -- hatch-vcs picks it up as the nearest tag and raises 'ValueError: Can't parse version from tag'. uv lock, uv sync and uv build all fail. My own step-0 note had predicted a benign 0.0.0 fallback and was WRONG: fallback_version covers a checkout with NO tag, not one whose nearest tag is unparseable. Fixed with git_describe_command matching v[0-9]*.[0-9]*.[0-9]* -- note a plain v[0-9]* does NOT work, because v1.2-powershell matches that glob.
(2) LICENSE must be restored after the overlay, not overwritten. The runbook said the overlay 'overwrites README.md and LICENSE' as if both were fine. README yes; LICENSE no -- the fork's copy is a re-indented Apache-2.0 (555 bytes smaller, body whitespace stripped) while the public repo already had the canonical text. Same licence, but there is no reason to ship a reformatted LICENSE to a public repo. Runbook now has the git checkout HEAD -- LICENSE step.

VERIFIED on the assembled repo: uv lock + sync resolve the library via the corrected ../ path, ruff clean, mypy clean on 63 files, the 4-file ci.yml gate passes (19 tests), and both collect sccm and convert sccm bind.

NEXT: all remaining actions are the owner's -- see the handoff list.
