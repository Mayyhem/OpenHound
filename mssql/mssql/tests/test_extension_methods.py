...
from openhound_mssql.main import app as ext_module

# Import the source module so registration side effects run and a failure to
# import surfaces as a real test error rather than being silently swallowed.
from openhound_mssql.source import source as mssql_source


def test_source_is_callable():
    assert callable(mssql_source), "openhound_mssql.source.source must be importable and callable"
