...
app = OpenHound("mssql", source_kind="Kind", help="OpenGraph collector for mssql")
...
def collect(ctx: CollectContext) -> DltSource:
    """Register a Typer CLI command that collects mssql resources and stores them (filtered) on disk.

    Args:
        ctx (CollectContext): Returns DLT pipeline context.
    """
    from .source import source as mssql_source
...
def convert(ctx: ConvertContext) -> DltSource:
    """Register a Typer CLI command that converts mssql resources to OpenGraph.
