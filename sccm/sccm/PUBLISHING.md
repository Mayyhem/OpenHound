...
Do **not** bring `TICKETS-BY-STATUS.md` across — it is generated from `gtk list`, self-declares as
generated, and its longest line is ~3,500 characters. It will be regenerated here on the next ticket
update, so create `.gitattributes` (the repo has none) with:

```gitattributes
# Generated by `gtk list` and regenerated on every ticket update, with ~3,500-character
# lines. Never merge it -- take whatever this side already has and let the next
