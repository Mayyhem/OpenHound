...
def test_force_tcp_defaults_resolve_to_tcp(monkeypatch):
    ...
    def _fake_resolve(qname, rdtype=None, *args, **kwargs):
        seen["tcp"] = kwargs.get("tcp")
        return []

    # Replace the underlying bound method the wrapper delegates to.
    monkeypatch.setattr(type(resolver), "resolve", _fake_resolve, raising=True)
    # The wrapper installed by make_resolver should inject tcp=True.
...
```python
    if force_tcp:
        ...
        def _resolve_tcp(qname, rdtype=None, *args, **kwargs):
            kwargs.setdefault("tcp", True)
            return _orig_resolve(qname, rdtype, *args, **kwargs)
