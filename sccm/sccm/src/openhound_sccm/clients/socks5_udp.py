...
Vendored from ``sccm/ConfigManBearPig/python/lib/socks5_udp.py`` (~214 LOC).
Only the logger name changed; no GraphStore references in the original.
...
(currently Phase 4 territory; Phase 2 only uses it as an optional unicast
fallback for DHCP discovery if ``--socks-proxy`` is set in the future).
...
class SOCKS5UDPRelay:
    ...
    def connect(self) -> None:
        ...
        logger.debug("SOCKS5: connecting to proxy %s:%d", self.proxy_host, self.proxy_port)
        ...
        if resp[1] != 0:
            raise SOCKS5Error("SOCKS5 proxy requires authentication (not supported)")
        ...
        if len(resp) < 10 or resp[0] != 5 or resp[1] != 0:
            ...
            raise SOCKS5Error(f"UDP ASSOCIATE failed (error code: {error_code})")
        ...
        if resp[3] == 1:  # IPv4
            ...
            self._relay_dst_port = int.from_bytes(resp[5 + domain_len : 5 + domain_len + 2], "big")
        else:
            raise SOCKS5Error(f"Unsupported relay address type: {resp[3]}")

        # If relay reports 0.0.0.0, use the proxy host instead
        if self._relay_dst in ("0.0.0.0", ""):
            self._relay_dst = self.proxy_host

        logger.debug("SOCKS5: UDP relay established at %s:%d", self._relay_dst, self._relay_dst_port)
    ...
    def sendto(self, data: bytes, dest_ip: str, dest_port: int) -> None:
        ...
        logger.debug(
            "SOCKS5: sent %d bytes -> %s:%d (via relay %s:%d)",
            len(data), dest_ip, dest_port, self._relay_dst, self._relay_dst_port,
        )

    def recvfrom(self, bufsize: int = 4096, timeout: float = 5.0) -> Optional[tuple[bytes, str, int]]:
        ...
        try:
            data = self._udp_sock.recv(bufsize + 10)
        ...
        if len(data) < 10:
            logger.debug("SOCKS5: received packet too small (%d bytes)", len(data))
            return None
        ...
        if data[3] == 1:  # IPv4
            ...
            sender_port = int.from_bytes(data[5 + domain_len : 5 + domain_len + 2], "big")
            payload = data[5 + domain_len + 2 :]
        else:
            logger.debug("SOCKS5: unsupported address type in response: %d", data[3])
            return None

        logger.debug("SOCKS5: received %d bytes from %s:%d", len(payload), sender_ip, sender_port)
