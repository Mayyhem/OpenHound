"""SCCM cryptographic operations.

Vendored from ``sccm/ConfigManBearPig/python/lib/sccm_crypto.py`` (~326 LOC).
Implements:
...
Phase 2 only uses ``mscrypt_derive_key_sha1`` and ``deobfuscate_secret_policy_blob``
indirectly (the DHCP discovery resource hands the bytes off to Phase 4 secret
material processing, which doesn't run yet). Kept verbatim — no GraphStore
references in this file to begin with — so future phases can import without
rewriting.
...
def mscrypt_derive_key_sha1(secret: bytes) -> bytes:
    """Derive AES/3DES key material from a secret using MS CryptDeriveKey + SHA-1.
...
def decrypt_media_variable_file(data: bytes, password: bytes) -> Optional[str]:
    ...
    try:
        ...
        logger.debug("Media variable file decryption failed: %s", e)
        return None
...
def derive_blank_decryption_key(encrypted_key: bytes) -> Optional[bytes]:
    ...
    try:
        ...
        logger.debug("Blank decryption key derivation failed: %s", e)
        return None
...
def deobfuscate_secret_policy_blob(blob_hex: str) -> Optional[str]:
    """Deobfuscate an SCCM secret policy blob (NAA / collection variables / TS secrets).
    ...
    try:
        ...
        if blob_prefix == b"\x89\x13":
            ...
            logger.warning("Unknown policy blob prefix: %s", blob_prefix.hex())
            return None
        ...
        logger.debug("Policy blob deobfuscation failed: %s", e)
        return None
...
def decrypt_cms_policy(policy_response: bytes, private_key) -> Optional[str]:
    ...
    try:
        ...
        logger.warning("CMS decryption requires pyasn1/pyasn1-modules: %s", e)
        return None
    ...
    try:
        ...
        enveloped, _ = decode(
            content.getComponentByName("content"),
        ...
        if key_algo == "rsaEncryption":
            ...
            logger.warning("Unsupported key encryption algorithm: %s", key_enc_oid)
            return None
        ...
        if body_algo == "des-ede3-cbc":
            ...
            cipher = Cipher(TripleDES(plaintext_key), modes.CBC(iv), backend=default_backend())
        elif body_algo == "aes256_cbc":
            cipher = Cipher(algorithms.AES(plaintext_key), modes.CBC(iv), backend=default_backend())
        else:
            logger.warning("Unsupported body encryption algorithm: %s", body_enc_oid)
            return None
        ...
        logger.debug("CMS policy decryption failed: %s", e)
        return None
