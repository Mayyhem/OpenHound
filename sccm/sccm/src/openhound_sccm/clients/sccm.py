"""SCCM Management Point client for the OpenHound SCCM extension.

Vendored verbatim from ``sccm/ConfigManBearPig/python/lib/sccm_client.py``
(~993 LOC), with two adaptations:

* Imports from ``lib.sccm_crypto`` rewritten to ``openhound_sccm.clients.sccm_crypto``.
* Module logger renamed from ``ConfigManBearPig`` to ``__name__`` so framework
  log filtering still works.

The class :class:`SCCMPolicyClient` is the load-bearing surface used by
Phase 3b's AdminService chain (HTTP-based MP request flow). Phase 3a does NOT
exercise this client directly — it's vendored here so the next session can
import without setup. No ``GraphStore`` references existed in the original;
this module returns plain dicts/lists that callers turn into JSONL rows.
...
from openhound_sccm.clients.sccm_crypto import (
    decrypt_cms_policy,
